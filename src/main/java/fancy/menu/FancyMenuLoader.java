...
public class FancyMenuLoader {
    ...
    public static boolean openMenu(Player player, FancyMenu inventory, boolean sound) {
        if (player.hasPermission(inventory.permission())) {
            if (sound) {
                player.playSound(player.getLocation(), fancy.PartlyFancy.getSound("sound.inventory.open"), 0.2f, 1f);
            }
            player.openInventory(inventory.getInventory());
            return true;
        } else {
            player.closeInventory();
            player.sendMessage(fancy.PartlyFancy.getPrefix() + fancy.PartlyFancy.getStringValue("message.menu.no-permission", "%player%-" + player.getDisplayName(), "%permission%-" + inventory.permission().getName()));
            if (sound) {
                player.playSound(player.getLocation(), fancy.PartlyFancy.getSound("sound.inventory.no-permission"), 0.5f, 1f);
            }
        }
        return false;
    }
    ...
    public static void closeMenu(Player player, boolean sound) {
        player.closeInventory();
        if (sound) {
            player.playSound(player.getLocation(), fancy.PartlyFancy.getSound("sound.inventory.close"), 0.2f, 1f);
        }
    }
